...
አል-መህዲ አዳዲስ</h1>
         ...
         <ul className="grid gap-x-8 gap-y-10 mt-16 sm:grid-cols-2 lg:grid-cols-3">
                    {
                        post.map((items, key) => (
                           <li className="w-full mx-auto group sm:max-w-sm shadow-md transition ease-in hover:-translate-y-2 hover:shadow-lg" key={key}>
  ...
  />
                                    ...
                                    <div className="mt-3 pb-2 space-y-2">
                                       ...
                                       <Link rel="stylesheet" to={`/singlepost/${items._id}`} > <h3 className="text-lg text-gray-800 duration-150 group-hover:text-indigo-600 font-semibold">
                                            {items.title}
                                        </h3></Link>
                                        <p className="text-gray-600 text-sm truncate max-w-[380px] duration-150 group-hover:text-gray-800">{items.content.substring(0, 100) + " ..."}</p>
                                    </div>
